...
contract Orders is DexzBase {
    ...
    uint public constant ORDERTYPE_BUY = 0x00;
    uint public constant ORDERTYPE_SELL = 0x01;
    uint public constant ORDERFLAG_BUYSELL_MASK = 0x01;
    // BK Default is to fill as much as possible
    uint public constant ORDERFLAG_FILL = 0x00;
    uint public constant ORDERFLAG_FILLALL_OR_REVERT = 0x10;
    uint public constant ORDERFLAG_FILL_AND_ADD_ORDER = 0x20;
    ...
    function getBestPrice(
        ...
        _key = (_orderType == ORDERTYPE_BUY)
            ? orderKeys[_pairKey][_orderType].last()
            : orderKeys[_pairKey][_orderType].first();
    }
    function getNextBestPrice(
        ...
        if (BokkyPooBahsRedBlackTreeLibrary.isEmpty(_x)) {
            _y = (_orderType == ORDERTYPE_BUY)
                ? orderKeys[_pairKey][_orderType].last()
                : orderKeys[_pairKey][_orderType].first();
        } else {
            _y = (_orderType == ORDERTYPE_BUY)
                ? orderKeys[_pairKey][_orderType].prev(_x)
                : orderKeys[_pairKey][_orderType].next(_x);
        }
    }
    ...
    function getOrderQueue(
        ...
        uint _orderType,
        uint _price
    ) public view returns (bool _exists, bytes32 _head, bytes32 _tail) {
        Orders.OrderQueue memory _orderQueue = orderQueue[_pairKey][_orderType][
            _price
        ];
        ...
        )
    {
        ...
        return (
            ...
            order.expiry,
            order.baseTokens,
            order.baseTokensFilled
        );
    }

    function _getBestMatchingOrder(
        uint _orderType,
        address baseToken,
        address quoteToken,
        uint price
        ...
        internal
        returns (uint _bestMatchingPriceKey, bytes32 _bestMatchingOrderKey)
    {
        bytes32 _pairKey = pairKey(baseToken, quoteToken);
        uint _matchingOrderType = inverseOrderType(_orderType);
        BokkyPooBahsRedBlackTreeLibrary.Tree storage priceKeys = orderKeys[
            _pairKey
        ][_matchingOrderType];
        // SKINNY2 if (priceKeys.initialised) {
        emit LogInfo(
            "getBestMatchingOrder: priceKeys.initialised",
            0,
            0x0,
            "",
            address(0)
        );
        _bestMatchingPriceKey = (_orderType == ORDERTYPE_BUY)
            ? priceKeys.first()
            : priceKeys.last();
        bool priceOk = BokkyPooBahsRedBlackTreeLibrary.isEmpty(
            _bestMatchingPriceKey
        )
            ? false
            : (_orderType == ORDERTYPE_BUY)
                ? _bestMatchingPriceKey <= price
                : _bestMatchingPriceKey >= price;
        while (priceOk) {
            emit LogInfo(
                "getBestMatchingOrder: _bestMatchingPriceKey",
                _bestMatchingPriceKey,
                0x0,
                "",
                address(0)
            );
            OrderQueue storage _orderQueue = orderQueue[_pairKey][
                _matchingOrderType
            ][_bestMatchingPriceKey];
            if (_orderQueue.exists) {
                emit LogInfo(
                    "getBestMatchingOrder: orderQueue not empty",
                    _bestMatchingPriceKey,
                    0x0,
                    "",
                    address(0)
                ...
                while (_bestMatchingOrderKey != ORDERKEY_SENTINEL) {
                    ...
                    emit LogInfo(
                        "getBestMatchingOrder: _bestMatchingOrderKey ",
                        order.expiry,
                        _bestMatchingOrderKey,
                        "",
                        address(0)
                    );
                    if (
                ...
                emit LogInfo(
                    "getBestMatchingOrder: orderQueue empty",
                    0,
                    0x0,
                    "",
                    address(0)
            ...
            _bestMatchingPriceKey = (_orderType == ORDERTYPE_BUY)
                ? priceKeys.next(_bestMatchingPriceKey)
                : priceKeys.prev(_bestMatchingPriceKey);
            priceOk = BokkyPooBahsRedBlackTreeLibrary.isEmpty(
                _bestMatchingPriceKey
            )
                ? false
                : (_orderType == ORDERTYPE_BUY)
    ...
    function _updateBestMatchingOrder(
        uint _orderType,
        address baseToken,
        address quoteToken,
        ...
        bool _orderFilled
    ) internal returns (bytes32 _orderKey) {
        bytes32 _pairKey = pairKey(baseToken, quoteToken);
        uint _matchingOrderType = inverseOrderType(_orderType);
        BokkyPooBahsRedBlackTreeLibrary.Tree storage priceKeys = orderKeys[
            _pairKey
        ...
        // SKINNY2 if (priceKeys.initialised) {
        emit LogInfo(
            "updateBestMatchingOrder: priceKeys.initialised",
            0,
            0x0,
            "",
            address(0)
        );
        uint priceKey = (_orderType == ORDERTYPE_BUY)
            ? priceKeys.first()
            : priceKeys.last();
        while (!BokkyPooBahsRedBlackTreeLibrary.isEmpty(priceKey)) {
            emit LogInfo(
                "updateBestMatchingOrder: priceKey",
                priceKey,
                0x0,
                "",
                address(0)
            ...
            if (_orderQueue.exists) {
                emit LogInfo(
                    "updateBestMatchingOrder: orderQueue not empty",
                    priceKey,
                    0x0,
                    "",
                    address(0)
                ...
                priceKeys.remove(priceKey);
                emit LogInfo(
                    "updateBestMatchingOrder: orderQueue empty",
                    0,
                    0x0,
                    "",
                    address(0)
            ...
            priceKey = (_orderType == ORDERTYPE_BUY)
                ? priceKeys.next(priceKey)
                : priceKeys.prev(priceKey);
            // SKINNY2 }
        }
    ...
    function _addOrder(
        ...
        OrderQueue storage _orderQueue = orderQueue[_pairKey][_orderType][
            price
        ];
        if (!_orderQueue.exists) {
            orderQueue[_pairKey][_orderType][price] = OrderQueue(
                true,
                ORDERKEY_SENTINEL,
                ORDERKEY_SENTINEL
            );
    ...
    function _removeOrder(bytes32 _orderKey, address msgSender) internal {
        ...
        OrderQueue storage _orderQueue = orderQueue[_pairKey][order.orderType][
            order.price
        ];
