...
contract Dexz is Orders, ApproveAndCallFallback {
    ...
    function receiveApproval(
        ...
        if (functionSignature == tradeSig) {
            ...
            require(
                _token == address(uint160(baseToken)) ||
                    _token == address(uint160(quoteToken))
            );
            require(_tokens >= baseTokens);
            _trade(
                TradeInfo(
                    _from,
                    orderFlag | ORDERFLAG_FILL_AND_ADD_ORDER,
                    orderFlag & ORDERFLAG_BUYSELL_MASK,
                    address(uint160(baseToken)),
                    address(uint160(quoteToken)),
                    price,
                    expiry,
                    baseTokens,
                    address(uint160(uiFeeAccount))
                )
            );
        }
    }
    ...
    function trade(
        ...
        payable
        returns (
            uint _baseTokensFilled,
            uint _quoteTokensFilled,
            uint _baseTokensOnOrder,
            bytes32 _orderKey
        )
    {
        return
        ...
        )
    {
        ...
        (matchingPriceKey, matchingOrderKey) = _getBestMatchingOrder(
            tradeInfo.orderType,
            tradeInfo.baseToken,
        ...
        while (
            matchingOrderKey != ORDERKEY_SENTINEL && tradeInfo.baseTokens > 0
        ) {
            uint _baseTokens;
            uint _quoteTokens;
            bool _orderFilled;
            ...
            (_baseTokens, _quoteTokens, _orderFilled) = calculateOrder(
                matchingOrderKey,
                tradeInfo.baseTokens,
                tradeInfo.taker
            );
        ...
        if (
            (tradeInfo.orderFlag & ORDERFLAG_FILLALL_OR_REVERT) ==
            ORDERFLAG_FILLALL_OR_REVERT
        ) {
            require(tradeInfo.baseTokens == 0);
        }
        if (
            tradeInfo.baseTokens > 0 &&
            ((tradeInfo.orderFlag & ORDERFLAG_FILL_AND_ADD_ORDER) ==
                ORDERFLAG_FILL_AND_ADD_ORDER)
        ) {
    ...
    function calculateOrder(
        bytes32 _matchingOrderKey,
        uint amountBaseTokens,
        address taker
    ) internal returns (uint baseTokens, uint quoteTokens, bool _orderFilled) {
        ...
        if (matchingOrder.orderType == ORDERTYPE_BUY) {
            emit LogInfo(
                "calculateOrder Maker Buy: matchingOrder.baseTokens",
                matchingOrder.baseTokens,
                0x0,
                "",
                address(0)
            );
            emit LogInfo(
                "calculateOrder Maker Buy: matchingOrder.baseTokensFilled",
                matchingOrder.baseTokensFilled,
                0x0,
                "",
                address(0)
            );
            emit LogInfo(
                "calculateOrder Maker Buy: amountBaseTokens",
                amountBaseTokens,
                0x0,
                "",
                address(0)
            );
            uint _availableBaseTokens = availableTokens(
                matchingOrder.baseToken,
                taker
            );
            emit LogInfo(
                "calculateOrder Maker Buy: availableTokens(matchingOrder.baseToken, taker)",
                _availableBaseTokens,
                0x0,
                "",
                taker
            );
            ...
            if (
                matchingOrder.baseTokens - matchingOrder.baseTokensFilled >
                _availableBaseTokens
            ) {
                matchingOrder.baseTokens =
                    _availableBaseTokens +
                    matchingOrder.baseTokensFilled;
                emit LogInfo(
                    "calculateOrder Maker Buy: matchingOrder.baseTokens reduced due to available tokens",
                    matchingOrder.baseTokens,
                    0x0,
                    "",
                    address(0)
                ...
                emit LogInfo(
                    "calculateOrder Maker Buy: matchingOrder.baseTokens NOT reduced due to available tokens",
                    matchingOrder.baseTokens,
                    0x0,
                    "",
                    address(0)
            ...
            emit LogInfo(
                "calculateOrder Maker Buy: baseTokens = baseTokens.min(availableTokens(matchingOrder.baseToken, taker))",
                baseTokens,
                0x0,
                "",
                taker
            ...
            emit LogInfo(
                "calculateOrder Maker Buy: quoteTokens = baseTokens x price / 1e18",
                (baseTokens * matchingOrder.price) / TENPOW18,
                0x0,
                "",
                address(0)
            );
            uint _availableQuoteTokens = availableTokens(
                matchingOrder.quoteToken,
                matchingOrder.maker
            );
            emit LogInfo(
                "calculateOrder Maker Buy: availableTokens(matchingOrder.quoteToken, matchingOrder.maker)",
                _availableQuoteTokens,
                0x0,
                "",
                matchingOrder.maker
            );
            if (
                matchingOrder.orderType == ORDERTYPE_BUY &&
                (matchingOrder.baseTokens - matchingOrder.baseTokensFilled) >
                _availableBaseTokens
            ) {}
            ...
            emit LogInfo(
                "calculateOrder Maker Buy: quoteTokens = quoteTokens.min(availableTokens(matchingOrder.quoteToken, matchingOrder.maker))",
                quoteTokens,
                0x0,
                "",
                matchingOrder.maker
            ...
            emit LogInfo(
                "calculateOrder Maker Buy: baseTokens = min(baseTokens, quoteTokens x 1e18 / price)",
                baseTokens,
                0x0,
                "",
                address(0)
            ...
            emit LogInfo(
                "calculateOrder Maker Sell: matchingOrder.baseTokens",
                matchingOrder.baseTokens,
                0x0,
                "",
                address(0)
            ...
            emit LogInfo(
                "calculateOrder Maker Sell: matchingOrder.baseTokensFilled",
                matchingOrder.baseTokensFilled,
                0x0,
                "",
                address(0)
            ...
            emit LogInfo(
                "calculateOrder Maker Sell: amountBaseTokens",
                amountBaseTokens,
                0x0,
                "",
                address(0)
            ...
            emit LogInfo(
                "calculateOrder Maker Sell: availableTokens(matchingOrder.baseToken, matchingOrder.maker)",
                _availableBaseTokens,
                0x0,
                "",
                matchingOrder.maker
            ...
            // Update maker matchingOrder with currently available tokens
            if (
                matchingOrder.orderType == ORDERTYPE_SELL &&
                (matchingOrder.baseTokens - matchingOrder.baseTokensFilled) >
                _availableBaseTokens
            ) {
                matchingOrder.baseTokens =
                    ...
                    matchingOrder.baseTokensFilled;
                emit LogInfo(
                    "calculateOrder Maker Sell: matchingOrder.baseTokens reduced due to available tokens",
                    matchingOrder.baseTokens,
                    0x0,
                    "",
                    address(0)
                ...
                emit LogInfo(
                    "calculateOrder Maker Sell: matchingOrder.baseTokens NOT reduced due to available tokens",
                    matchingOrder.baseTokens,
                    0x0,
                    "",
                    address(0)
            ...
            emit LogInfo(
                "calculateOrder Maker Sell: baseTokens = baseTokens.min(availableTokens(matchingOrder.baseToken, matchingOrder.maker))",
                baseTokens,
                0x0,
                "",
                matchingOrder.maker
            ...
            emit LogInfo(
                "calculateOrder Maker Sell: quoteTokens = baseTokens x price / 1e18",
                (baseTokens * matchingOrder.price) / TENPOW18,
                0x0,
                "",
                address(0)
            ...
            emit LogInfo(
                "calculateOrder Maker Sell: availableTokens(matchingOrder.quoteToken, matchingOrder.maker)",
                _availableQuoteTokens,
                0x0,
                "",
                taker
            ...
            emit LogInfo(
                "calculateOrder Maker Sell: quoteTokens = quoteTokens.min(availableTokens(matchingOrder.quoteToken, taker))",
                quoteTokens,
                0x0,
                "",
                taker
            ...
            emit LogInfo(
                "calculateOrder Maker Sell: baseTokens = min(baseTokens, quoteTokens x 1e18 / price)",
                baseTokens,
                0x0,
                "",
                address(0)
    ...
    function transferTokens(
        ...
        if (orderType == ORDERTYPE_BUY) {
            ...
            emit Trade(
                ...
                tradeInfo.baseToken,
                tradeInfo.quoteToken,
                _baseTokens - _takerFeeInTokens,
                _quoteTokens,
                _takerFeeInTokens,
                0,
                __orderBaseTokensFilled
            );
            transferFrom(
                tradeInfo.quoteToken,
                tradeInfo.taker,
            ...
            if (_takerFeeInTokens > 0) {
                if (
                    feeAccount == tradeInfo.uiFeeAccount ||
                    _takerFeeInTokens == 1
                ) {
                    transferFrom(
                        tradeInfo.baseToken,
                    ...
                    transferFrom(
                        ...
                        maker,
                        feeAccount,
                        _takerFeeInTokens - _takerFeeInTokens / 2
                    );
                }
            }
        } else {
            ...
            emit Trade(
                ...
                tradeInfo.quoteToken,
                _baseTokens,
                _quoteTokens - _takerFeeInTokens,
                _takerFeeInTokens,
                0,
                __orderBaseTokensFilled
            );
